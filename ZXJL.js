...
async function getCookie() {
    const token = $request.headers["wxToken"] || $request.headers["wxToken"];
    if (!token) {
        return
    }
    const newData = {"token": token};
    const index = ZXJL.findIndex(e => e.token == newData.token);
    if (index !== -1) {
        if (ZXJL[index].token == newData.token) {
            ...
            $.msg($.name, `🎉用户${newData.token}更新成功!`, ``);
        }
    } else {
        ZXJL.push(newData)
        console.log(newData.token)
        $.msg($.name, `🎉新增用户${newData.token}成功!`, ``);
    }
    $.setjson(ZXJL, "ZXJL");
}
