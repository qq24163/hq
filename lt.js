/*
中国联通 v1.09 + 自动提取Token整合版 + 新活动

包含以下功能:
首页签到, 联通祝福, 立减金打卡，权益超市，安全管家任务, 联通云盘，新活动(福利狂欢·天天抽百元话费)
...
// ============================================
// 新活动：福利狂欢·天天抽百元话费 (202511-2)
// ============================================

async wo4g_task() {
    this.log("============= 福利狂欢·天天抽百元话费 2026年6月30日截止 =============");
    ...
    try {
        ...
        this.log("============= 福利狂欢·天天抽百元话费执行完毕 =============");
        
    } catch (e) {
        this.log(`执行异常: ${e.message}`);
    }
}
...
async wo4g_handle_sign(session) {
    ...
    if (signRecord?.code === "1") {
        ...
        if (!todaySigned) {
            this.log(`今日未签到（当前连续${continuousNum}天）`);
            ...
            if (signResult?.code === "1") {
                ...
                if (rewardResult?.code === "1" && rewardResult.data?.value) {
                    this.log(`✅ 领取签到能量成功，获得 ${rewardResult.data.value} 点能量`, { notify: true });
                } else {
                    this.log(`❌ 领取签到能量失败: ${rewardResult?.msg || "未知错误"}`);
                }
                
                this.log(`✅ 完成第${continuousNum + 1}天连续签到`, { notify: true });
            } else {
                this.log(`❌ 签到失败: ${signResult?.msg || "未知错误"}`);
            }
        } else {
            this.log(`今日已签到（连续${continuousNum}天）`);
        }
    }
}
...
async wo4g_handle_tasks(session) {
    ...
    if (taskState?.code === "1" && Array.isArray(taskState.data)) {
        ...
        this.log(`发现 ${unfinishedTasks.length} 个其他可领取的任务`);
        ...
        for (const task of unfinishedTasks) {
            ...
            if (rewardResult?.code === "1" && rewardResult.data?.value) {
                this.log(`✅ 领取任务 ${task.taskId} 成功，获得 ${rewardResult.data.value} 积分`, { notify: true });
            }
...
async wo4g_handle_draw(session) {
    ...
    this.log(`发现 ${drawTimes} 次抽奖机会，开始抽奖...`);
    
    let successCount = 0;
    let failCount = 0;
    
    for (let i = 0; i < drawTimes; i++) {
        this.log(`执行第 ${i + 1}/${drawTimes} 次抽奖...`);
        ...
        if (drawResult?.code === "1") {
            if (drawResult.data?.lottPrize) {
                ...
                this.log(`🎉 第${i + 1}次抽奖成功，获得: ${prize.prizeName} (${prize.prizeDesc})`, { notify: true });
                successCount++;
            } else {
                this.log(`第${i + 1}次抽奖未中奖`);
                successCount++;
            }
        } else {
            this.log(`❌ 第${i + 1}次抽奖失败`);
            failCount++;
        }
    ...
    this.log(`抽奖完成 ${successCount}次成功，${failCount}次失败`);
}
        ...
        if (signRecord?.code === "1") {
            ...
            if (!todaySigned) {
                ...
                if (signResult?.code === "1") {
                    ...
                    if (rewardResult?.code === "1" && rewardResult.data?.value) {
                        this.log(`✅ 签到成功，获得 ${rewardResult.data.value} 点能量 (连续${continuousNum + 1}天)`, { notify: true });
                    }
                }
            } else {
                const continuousNumResult = await this.wo4g_api_call2(
    ...
    try {
        ...
        if (task) {
            if (task.finished && !task.rewarded) {
                ...
                if (rewardResult?.code === "1" && rewardResult.data?.value) {
                    this.log(`✅ 领取浏览任务奖励成功，获得 ${rewardResult.data.value} 点能量`, { notify: true });
                }
                ...
                if (browseSignResult?.code === "1") {
                    ...
                    if (rewardResult?.code === "1" && rewardResult.data?.value) {
                        this.log(`✅ 浏览任务完成，获得 ${rewardResult.data.value} 点能量`, { notify: true });
                    }
                }
            } else {
                this.log("浏览任务已完成且奖励已领取");
            }
        }
    } catch (e) {
        this.log(`处理浏览任务异常: ${e.message}`);
    }
}
        ...
        if (taskState?.code === "1" && Array.isArray(taskState.data)) {
            ...
            for (const task of taskState.data) {
                if (task && task.finished && !task.rewarded) {
                    ...
                    if (rewardResult?.code === "1" && rewardResult.data?.value) {
                        this.log(`✅ 领取任务 ${task.taskId} 奖励，获得 ${rewardResult.data.value} 积分`, { notify: true });
                        claimedCount++;
                    }
                    await appName.wait(1000);
                }
            ...
            if (claimedCount === 0) {
                this.log("没有发现可领取的其他任务");
            }
        }
    } catch (e) {
        this.log(`处理其他任务异常: ${e.message}`);
    }
}
...
async wo4g_api_call2(apiPath, body, session) {
  ...
  async userTask() {
    ...
    await this.wo4g_task();   //福利狂欢
    await this.wo4g_task2(); // 签到领能量
  }
  async userTestTask() {
    appName.log("\n------------------ 账号[" + this.index + "] ------------------");
  }
