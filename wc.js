...
// 接口1: vapp.taizhou.com.cn/api/user_mumber/account_detail (GET) - 捕获请求头字段
// 接口2: xmt.taizhou.com.cn/prod-api/user-read/app/login - 捕获User-Agent2
...
(function() {
    ...
    console.log(`[WangChao] 接口: ${isVapp ? 'vapp(账号详情-GET)' : 'xmt(登录接口)'}`);
    ...
    try {
        ...
        if (isVapp) {
            ...
            console.log(`[WangChao] 响应内容预览: ${responseBody.substring(0, 200)}`);
            
            let mobile = extractMobile(responseBody);
            if (!mobile) {
                console.log('[WangChao] 未找到手机号');
                // 尝试从请求头或URL中获取
                mobile = extractMobileFromHeaders(headers);
                if (!mobile) {
                    console.log('[WangChao] 无法获取手机号，跳过保存');
                    $done({});
                    return;
                }
            }
    ...
    function extractMobile(responseBody) {
        try {
            ...
            // 如果还没找到，尝试查找包含mobile的字段
            if (!mobile) {
                const findMobile = (obj, path = '') => {
                    for (let key in obj) {
                        if (typeof obj[key] === 'object' && obj[key] !== null) {
                            findMobile(obj[key], `${path}${key}.`);
                        } else if ((key.includes('mobile') || key.includes('phone')) && obj[key]) {
                            mobile = obj[key];
                            console.log(`[WangChao] 找到手机号字段: ${path}${key} = ${mobile}`);
                        }
                    }
                };
                findMobile(jsonData);
    ...
    // ========== 从请求头中提取手机号（备用） ==========
    function extractMobileFromHeaders(headers) {
        // 尝试从Authorization或Cookie中提取
            ...
            try {
                ...
                if (parts.length === 3) {
                    ...
                    if (payload.mobile || payload.phone) {
                        return payload.mobile || payload.phone;
                    }
                }
            } catch (e) {}
        }
    ...
    function updateUserAgent2(mobile, newUserAgent2) {
        ...
        if (!storedData) {
            ...
            $notify("⚠️ 王朝", "存储为空", "请先通过账号详情接口初始化");
            return;
        }
        ...
        if (updated) {
            ...
            $notify("⚠️ 王朝", "未找到对应账号", `手机号 ${mobile} 不存在，请先通过账号详情接口登录`);
        }
    }
})();